...
void updateView(bool selected, int item, float velocity)
{
...
int readSelector(int curr, int max)
{
	...
	int res = newPosition > oldPosition ? 1 : newPosition < oldPosition ? -1 : 0;
...
{
	// Update view
	updateView(active, curr_item, 0.5);

	// Control
	debouncer.update();
	active = !debouncer.read();

	curr_item = readSelector(curr_item, MAX_ITEMS);
